...
function aiContent(idx){
  switch(idx){
    ...
    case 8: return `<div class="toolbar"><div class="search">${icon('search',{size:17})}<input placeholder="自然文で検索（例: 倉庫 高天井 200W相当 防塵）" value="倉庫 高天井 200W相当"></div><button class="btn">${icon('ai',{size:15})}AI検索</button></div>
      ...
      <div class="grid">${['ERE-2200','ERG-5512','ERK-1041'].map(s=>{const p=prod(s);return `<div class="pcard"><div class="thumb">${icon(iconFor(p.cat),{size:40})}</div><div class="body"><div class="sku">${p.sku} ・ ${p.cat}</div><div class="nm">${p.name}</div><div class="price">${yen(p.price)}<span class="u">/ 税抜</span></div><div class="pfoot"><span class="legend">AI適合度 高</span><button class="btn sm" onclick="addCart('${p.sku}')">${icon('plus',{size:13})}カート</button></div></div></div>`;}).join('')}</div>`;
